...
Свои баллы можно посмотреть перейдя в локальный клон этого репозитория
и выполнив команду:

    ./score.sh фамилия

Рейтинг можно посмотреть выполнив команду:
...
Программа выполняет то что указано в условии. Без данного критерия
...
(Без данного критерия следующие не учитываются)
...
Проблемы решаются исправлением ошибок или архитектуры, а не вбиванием
костылей.

### 3.10 Прочие проблемы, мешающие понимнию кода или поддержке

Поскольку все возможные способы превратить сопровождение кода в ад
...
4.1 Описание программы (что она делает и для чего нужна) в файле README = 47

4.2 При запуске с аргументом -h (или при запуске без аргументов если
использование без аргументов не предполагается) выводится руководство по
использованию, после чего программа завершается = 38
